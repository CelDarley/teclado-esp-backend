...
class User(models.Model):
    ...
    is_active_user = models.BooleanField(default=True)  # type: ignore
...
class AccessLog(models.Model):
    ...
    success = models.BooleanField(default=False)  # type: ignore
    ...
    def __str__(self):
        ...
        try:
            user_name = getattr(self.user, 'username', 'Desconhecido') if self.user else 'Desconhecido'
        except:
            user_name = 'Desconhecido'
        return f"{user_name} - {str(self.access_time)[:16]} - {status}"
